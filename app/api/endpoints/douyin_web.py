...
async def fetch_user_mix_videos(request: Request,
                                mix_id: str = Query(examples=["7348687990509553679"], description="合辑id/Mix id"),
                                max_cursor: int = Query(default=0, description="最大游标/Maximum cursor"),
                                counts: int = Query(default=20, description="每页数量/Number per page")):
    """
    # [中文]
...
async def fetch_user_live_videos(request: Request,
                                 webcast_id: str = Query(examples=["285520721194"],
...
async def fetch_user_live_videos_by_room_id(request: Request,
                                            room_id: str = Query(examples=["7318296342189919011"],
...
async def fetch_live_gift_ranking(request: Request,
                                  room_id: str = Query(examples=["7356585666190461731"],
                                                       description="直播间room_id/Room room_id"),
...
async def fetch_live_room_product_result(request: Request,
                                         cookie: str = Query(examples=["YOUR_COOKIE"],
                                                             description="用户网页版抖音Cookie/Your web version of Douyin Cookie"),
                                         room_id: str = Query(examples=["7356742011975715619"],
                                                              description="直播间room_id/Room room_id"),
                                         author_id: str = Query(examples=["2207432981615527"],
...
async def fetch_video_comments_reply(request: Request,
                                     item_id: str = Query(examples=["7354666303006723354"], description="作品id/Video id"),
                                     comment_id: str = Query(examples=["7354669356632638218"],
...
async def get_sec_user_id(request: Request,
                          url: str = Query(
                              examples=["https://www.douyin.com/user/MS4wLjABAAAANXSltcLCzDGmdNFI2Q_QixVTr67NiYzjKOIP5s03CAE"])):
    """
    # [中文]
    ### 用途:
    - 提取单个用户id
...
async def get_aweme_id(request: Request,
                       url: str = Query(examples=["https://www.douyin.com/video/7298145681699622182"])):
    """
    # [中文]
    ### 用途:
    - 提取单个作品id
...
async def get_webcast_id(request: Request,
                         url: str = Query(examples=["https://live.douyin.com/775841227732"])):
    """
    # [中文]
    ### 用途:
    - 提取列表直播间号
