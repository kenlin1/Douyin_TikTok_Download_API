...
async def fetch_one_video(request: Request,
                            itemId: str = Query(examples=["7339393672959757570"], description="作品id/Video id")):
    """
    # [中文]
    ### 用途:
    - 获取单个作品数据
...
async def fetch_user_post(request: Request,
                          secUid: str = Query(examples=["MS4wLjABAAAAv7iSuuXDJGDvJkmH_vz1qkDZYo1apxgzaxdBSeIuPiM"],
...
async def fetch_user_like(request: Request,
                          secUid: str = Query(
                              examples=["MS4wLjABAAAAq1iRXNduFZpY301UkVpJ1eQT60_NiWS9QQSeNqmNQEDJp0pOF8cpleNEdiJx5_IU"],
                              description="用户secUid/User secUid"),
                          cursor: int = Query(default=0, description="翻页游标/Page cursor"),
                          count: int = Query(default=35, description="每页数量/Number per page"),
                          coverFormat: int = Query(default=2, description="封面格式/Cover format")):
...
async def fetch_user_collect(request: Request,
                             cookie: str = Query(examples=["Your_Cookie"], description="用户cookie/User cookie"),
                             secUid: str = Query(examples=["Your_SecUid"], description="用户secUid/User secUid"),
                             cursor: int = Query(default=0, description="翻页游标/Page cursor"),
...
async def fetch_user_mix(request: Request,
                         mixId: str = Query(examples=["7101538765474106158"],
...
async def fetch_post_comment(request: Request,
                             aweme_id: str = Query(examples=["7304809083817774382"], description="作品id/Video id"),
                             cursor: int = Query(default=0, description="翻页游标/Page cursor"),
...
async def fetch_post_comment_reply(request: Request,
                                   item_id: str = Query(examples=["7304809083817774382"], description="作品id/Video id"),
                                   comment_id: str = Query(examples=["7304877760886588191"],
...
async def get_sec_user_id(request: Request,
                          url: str = Query(
                              examples=["https://www.tiktok.com/@tiktok"],
...
async def get_aweme_id(request: Request,
                       url: str = Query(
                           examples=["https://www.tiktok.com/@owlcitymusic/video/7218694761253735723"],
